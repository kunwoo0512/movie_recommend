...
아쉬운 점도 존재하는데 현재는 흐름곡선 등의 데이터를 직접 가공하는 과정에서 많은 양의 데이터를 수집하진 못하였고 741개의 영화 정보 데이터셋을 사용하였기 때문에 검색어에 대해서 동떨어진 결과를 반환한다는 아쉬움이 있다. 또 ‘28일 후’와 같은 영화는 대중들에게 좀비 영화로 널리 알려져있는데, ‘좀비’라는 키워드를 사용하여 검색하여도 검색 결과에 반환되지 않는 모습을 보였다. 이는 줄거리 데이터에 ‘감염’이라는 단어만 사용되었고 ‘좀비’라는 단어는 사용되지 않았기 때문으로 보인다. 그러나 이런 부분은 더 많은 데이터 셋의 확보와 줄거리를 어떤 방식으로 정리하느냐에 따라 개선될 여지가 있을 것으로 보여진다.
